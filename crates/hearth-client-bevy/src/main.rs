...
fn spawn_action_panel(
    ...
    if view.mulligan_eligible.is_empty() {
        ...
        for action in filtered.iter().skip(start).take(ACTIONS_PER_PAGE) {
            let label = command_label(session, view, &action.command, action.mana_cost);
...
fn command_label(
    ...
    let entity_name = |entity: EntityId| {
        view.entity(entity)
            .map(|view| session.card_name(&view.card_id))
            .unwrap_or_else(|| format!("#{entity}"))
    };
    ...
    match command {
        ...
        PlayerCommand::Mulligan { replace } => {
            let cards = replace
                ...
                .join(", ");
            match locale {
                hearth_core::Locale::EnUs => format!("Replace {cards}"),
                hearth_core::Locale::ZhCn => format!("替换 {cards}"),
                hearth_core::Locale::ZhTw => format!("替換 {cards}"),
            }
        }
        PlayerCommand::PlayCard { card, target: to } => match locale {
            hearth_core::Locale::EnUs => {
                format!("Play {}{}  ({mana_cost})", entity_name(*card), target(*to))
            }
            hearth_core::Locale::ZhCn => {
                format!(
                    "打出 {}{}  （{mana_cost}）",
        ...
        PlayerCommand::TradeCard { card } => match locale {
            hearth_core::Locale::EnUs => format!("Trade {}  (1)", entity_name(*card)),
            hearth_core::Locale::ZhCn => format!("交易 {}  （1）", entity_name(*card)),
            hearth_core::Locale::ZhTw => format!("交換 {}  （1）", entity_name(*card)),
        },
...
fn card_action_label(locale: hearth_core::Locale, action: &str) -> String {
    match action {
...
mod tests {
    ...
    fn generic_card_actions_have_readable_localized_labels() {
        assert_eq!(
            card_action_label(hearth_core::Locale::EnUs, "titan_1"),
            "Titan Ability I"
        );
        assert_eq!(
            card_action_label(hearth_core::Locale::ZhCn, "titan_2"),
            "泰坦技能二"
        );
        assert_eq!(
            card_action_label(hearth_core::Locale::ZhTw, "forge"),
            "鑄造"
        );
        assert_eq!(
            card_action_label(hearth_core::Locale::EnUs, "future_action"),
            "future_action"
        );
    }
