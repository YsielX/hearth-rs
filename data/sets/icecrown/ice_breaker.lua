...
card.triggers={{event="damaged",timing="after",active_zones={"weapon"},condition=function(ctx,self,event)local p=ctx:controller(self);return event.source==ctx:player(p).hero and ctx:player(p).weapon==self and event.amount>0 and ctx:entity(event.target).type=="minion"and ctx:entity(event.target).frozen end,effect=function(ctx,self,event)ctx:destroy(event.target)end}}
return card
