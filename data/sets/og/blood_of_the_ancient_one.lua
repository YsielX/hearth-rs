...
function card.merge_bloods(ctx,self)local found={};for _,m in ipairs(ctx:friendly_minions(self))do if ctx:entity(m).card_id=="OG_173"then found[#found+1]=m end end;if #found>=2 then local pos=math.min(ctx:board_position(found[1]),ctx:board_position(found[2]));ctx:move(found[1],"removed");ctx:move(found[2],"removed");ctx:summon_at(ctx:controller(self),"OG_173a",pos)end end
