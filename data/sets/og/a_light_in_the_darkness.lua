...
function card.receive_minion(ctx,self,id)cardlib.effects.give_card(ctx, ctx:controller(self),id)end
card.triggers={{event="card_created",timing="after",active_zones={"graveyard"},condition=function(ctx,self,event)return event.source==self end,effect=function(ctx,self,event)cardlib.effects.buff(ctx, event.entity,2,2)end}}
return card
