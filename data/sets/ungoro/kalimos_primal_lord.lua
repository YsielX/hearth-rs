...
function card.kalimos_chosen(ctx,self,id) ctx:cast_spell(ctx:controller(self),id,nil) end
card.tokens={
 ...
 {id="UNG_211b",name="Invocation of Water",text="Restore 12 Health to your hero.",set="UNGORO",type="spell",class="shaman",collectible=false,cost=0,on_play=function(ctx,self) local p=ctx:controller(self);ctx:heal(ctx:player(p).hero,12) end},
 {id="UNG_211c",name="Invocation of Fire",text="Deal 6 damage to the enemy hero.",set="UNGORO",type="spell",class="shaman",collectible=false,cost=0,on_play=function(ctx,self) local p=ctx:opponent(ctx:controller(self));ctx:damage_ignoring_spell_damage(ctx:player(p).hero,6) end},
 {id="UNG_211d",name="Invocation of Air",text="Deal 3 damage to all enemy minions.",set="UNGORO",type="spell",class="shaman",collectible=false,cost=0,on_play=function(ctx,self) local t={} for _,e in ipairs(ctx:enemy_characters(self)) do if ctx:entity(e).type=="minion" then t[#t+1]={e,3} end end ctx:damage_batch_ignoring_spell_damage(t) end},
}
return card
