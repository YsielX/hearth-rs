...
local card = { api_version = 1, id = "UNG_082", name = "Thunder Lizard",
    text = "<b>Battlecry</b>: If you played an Elemental last turn, <b>Adapt</b>.", set = "UNGORO",
    type = "minion", rarity = "common", cost = 3, attack = 3, health = 3, tags = { "beast" }, keywords = { "battlecry" } }
function card.on_battlecry(ctx, self)
    for _, card_id in ipairs(ctx:cards_played_last_turn(ctx:controller(self))) do
        if elemental(ctx:card_definition(card_id)) then ctx:discover_cards(ctx:controller(self), "Adapt", options, 3, "adapted"); return end
    end
end
