...
local kobold={id="LOOT_998k",name="Golden Kobold",text="<b>Taunt</b>\n<b>Battlecry:</b> Replace your hand with <b>Legendary</b> minions. They cost (1) less.",set="LOOTAPALOOZA",type="minion",collectible=false,cost=3,attack=6,health=6,keywords={"taunt","battlecry"}}
...
function kobold.replace_with_legendary(ctx,self,x)ctx:transform(x.target,x.id);ctx:modify(x.target,{stat="cost",operation="add",value=-1});ctx:set_data(self,"kobold_done:"..x.target,1);ctx:continue_with("replace_kobold_card")end
local wand={id="LOOT_998l",name="Wondrous Wand",text="Draw 3 cards. Set their Cost to (1).",set="LOOTAPALOOZA",type="spell",collectible=false,cost=3}
...
function wand.wand_discount(ctx,self,e)if ctx:entity(e).zone=="hand"then ctx:modify(e,{stat="cost",operation="set",value=1})end;local n=ctx:get_data(self,"wand_left")-1;ctx:set_data(self,"wand_left",n);if n>0 then ctx:continue_with("wand_draw")end end
