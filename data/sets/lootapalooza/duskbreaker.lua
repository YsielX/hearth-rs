...
local card={api_version=1,id="LOOT_410",name="Duskbreaker",text="<b>Battlecry:</b> If you're holding\na Dragon, deal 3 damage to all other minions.",set="LOOTAPALOOZA",type="minion",class="priest",rarity="rare",cost=4,attack=3,health=3,tags={"dragon"},keywords={"battlecry"}}
function card.on_battlecry(ctx,self)
    local held=false;for _,e in ipairs(ctx:hand(ctx:controller(self)))do if dragon(ctx,e)then held=true;break end end
    if held then local targets={};for _,e in ipairs(ctx:minions())do if e~=self then targets[#targets+1]=e end end;cardlib.effects.damage_all(ctx, targets,3)end
end
return card
