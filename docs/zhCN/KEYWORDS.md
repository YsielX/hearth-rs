...
| `echo` | 回响 | 创建继承打出费用的本回合临时副本，并对回响副本执行官方最低 1 费规则 |
...
[`data/keyword_examples`](../../data/keyword_examples/README.md) 中的清单把 68 个模块一一映射到暴雪官方卡牌库中已实现的代表牌。对应测试检查每个关键词的真实状态变化或效果，而不只是检查卡牌 ID 能否加载。
