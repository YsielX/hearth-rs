...
| `echo` | 迴響 | 建立繼承打出費用的本回合臨時副本，並對迴響副本執行官方最低 1 費規則 |
...
[`data/keyword_examples`](../../data/keyword_examples/README.md) 中的清單把 68 個模組一一對應到暴雪官方卡牌庫中已實作的代表牌。對應測試檢查每個關鍵詞的真實狀態變化或效果，而不只是檢查卡牌 ID 能否載入。
