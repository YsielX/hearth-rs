...
├── hearth-cli/            # 互動/Bot/Fuzzer 對局執行器和 fuzz 子命令
├── hearth-bot/            # 不讀取隱藏資訊的確定性基礎 Bot
└── hearth-fuzz/           # 獨立的確定性狀態機 Fuzzer
...
目前倉庫保留 1386 個官方卡牌、衍生物、英雄和英雄技能定義，覆蓋 45 個 set；其中包括完整的 30 張《納克薩瑪斯的詛咒》、123 張《哥哥打地地》、31 張《黑石山的火焰》、132 張《銀白聯賽》、45 張《探險者協會》、134 張《古神碎碎念》、45 張《夜夜卡拉贊》、132 張《黑街英雄之加基森風雲》、135 張《勇闖安戈洛》和 135 張《冰封王座的騎士》，以及 11 個基礎職業英雄技能、每個已審計構築關鍵詞至少一張完整實作的官方代表牌，並完整實作一套經典洞穴任務賊。這仍是代表性規則語料，不是完整官方卡池。
...
確定性狀態機 Fuzzer 已獨立放在 [`crates/hearth-fuzz/`](crates/hearth-fuzz/README.md)，普通 `cargo test` 不會啟動 fuzz campaign。它會產生職業合法牌組、抽取引擎列舉的合法操作、在每一步校驗狀態不變量，並將最終狀態與 replay 比對。可以直接透過主 CLI 執行：

```bash
cargo run -p hearth-cli --release -- fuzz --seeds 100 --steps 180
```

也可以透過 `cargo run -p hearth-fuzz --release -- ...` 執行專用二進位；兩個入口共用同一份實作。

## 邊界

這仍是規則原型，不是完整爐石服務端。關鍵詞層已經覆蓋構築模式詞表，但卡牌庫仍是 1386 個官方定義，並不等於完整官方卡池；酒館戰棋、傭兵戰紀的模式專屬關鍵詞也不在本 CLI 對戰規則範圍內。新增一種現有鉤子無法描述的基礎規則時，應優先增加通用規則鉤子或原子效果，而不是在 Rust 中判斷具體卡牌或關鍵詞名。
